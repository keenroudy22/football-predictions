...
ESPN currently rejects long historical scoreboard date ranges. The baseline refresh reads the prior NFL season by season identifier, the prior college season by individual weeks, and the current season by calendar-month windows. Every current window must succeed before any public schedule or forecast is replaced. ESPN can cap college historical weekly responses at 25 games; `slate.json` records this as `trainingCoverage: limited weekly provider sample`. That training set is incomplete and should never be presented as a full census or proof of model accuracy.
