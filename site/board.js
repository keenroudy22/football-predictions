(function (root, factory) {
  ...
  function rows(state = {}) {
    ...
    const unique = new Map(), priority = row => row.official ? 6 : row.sourceType === 'market' && row.quoteType === 'sportsbook' && ['current', 'active'].includes(row.status) && validOdds(row.odds) ? 5 : row.sourceType === 'research' ? 3 : row.sourceType === 'market' ? 2 : 1;
    for (const row of out.values()) {
      const fingerprint = JSON.stringify([row.gameId, row.athleteId || normalize(row.subject), normalize(row.market), normalize(row.marketWindow), normalize(row.direction), row.line, normalize(row.book), row.odds, time(row.quotedAt || row.observedAt)]);
      const previous = unique.get(fingerprint);
      if (!previous || priority(row) > priority(previous)) unique.set(fingerprint, row);
    }
  ...
  function summarizeTicket(selected, stake = 1, mode = 'units', unitValue = 1, now = Date.now()) {
    ...
    else if (new Set(selected.map(row => normalize(row.book))).size !== 1) reason = 'Mixed sportsbooks require one combined sportsbook quote.';
  ...
  function detailHTML(row, state = {}, helpers = {}) {
