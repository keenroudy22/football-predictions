...
function research(kind){
  ...
  const favoriteCards=favorites.length?`<section class="favorite-lines"><div class="section-head"><div><h3>Daily favorites</h3><p>Selected when published. Each selection is counted once in the record.</p></div><span class="count">${core.filter(FootballRecords.favorite).length} LINES</span></div><div class="cards">${favorites.map(pickCard).join('')}</div></section>`:'';
...
function parlayTier(p){
  if(p.parlayType==='longshot'||p.longshot===true||/longshot/i.test(p.title)||Number(p.odds)>=1000)return {rank:2,label:'Longshot'};
