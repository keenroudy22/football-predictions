...
let state={league:localStorage.getItem('football-league')||'NFL',view:'scores',week:weekOf(new Date()),query:'',filter:'all',recordScope:'favorites',recordLeague:'NFL',recordWeek:'total',slate:null,forecasts:[],reports:[],history:null};
...
function research(kind){
  ...
  const favorites=core.filter(FootballRecords.favorite),otherCore=core.filter(p=>!FootballRecords.favorite(p));
  const watchItems=new Map();selectedReports.forEach(r=>(r.gameWatch||[]).forEach(w=>watchItems.set(w.id,{...w,publishedAt:r.publishedAt})));
  ...
  const favoriteCards=favorites.length?`<section class="favorite-lines"><div class="section-head"><div><h3>Daily favorites</h3><p>Selected when published. The same lines appear once in the official record.</p></div><span class="count">${favorites.length} LINES</span></div><div class="cards">${favorites.map(pickCard).join('')}</div></section>`:'';
  const cards=kind==='props'?`${favoriteCards}${otherCore.length?`<section class="all-lines"><div class="section-head"><div><h3>Other official player lines · by position</h3><p>All remaining published selections for this week, including historical imports. Favorites above are included in the ${all.length}-line official total.</p></div><span class="count">${otherCore.length} LINES</span></div>${positionCards}</section>`:''}${risky.length?`<section class="risky-lines"><div class="section-head"><div><h3>Risky lines</h3><p>Higher-variance props and longshot-style player outcomes. They are tracked separately from the main card.</p></div><span class="count">${riskyActive.length} ACTIVE</span></div><div class="cards">${risky.map(pickCard).join('')}</div></section>`:''}`:all.length?`<div class="cards">${all.map(pickCard).join('')}</div>`:'';
...
function navigate(){const view=location.hash.slice(1);state.view=view.startsWith('game/')?'game':['scores','props','parlays','record'].includes(view)?view:'scores';if(state.view==='game'){const g=gameMap().get(decodeURIComponent(view.slice(5)));if(g){state.league=g.league;state.week=weekOf(g.kickoff);setupWeeks()}}if(state.slate)render()}
async function init(){
