...
function gameLineCatalog(g){
  const board=typeof window!=='undefined'?window.KeenBoard:null;
  const rows=(board?.rows(state)||[]).filter(row=>row.gameId===g.id&&row.position!=='Game').sort((a,b)=>Number(b.favorite)-Number(a.favorite)||Number(b.official)-Number(a.official)||Number(board.quoteState(b).eligible)-Number(board.quoteState(a).eligible)||(a.position||'Other').localeCompare(b.position||'Other')||(a.subject||'').localeCompare(b.subject||''));
  if(!rows.length)return '<p>No sourced player lines have been collected for this game yet.</p>';
  const cards=rows.map(row=>{const quote=board.quoteState(row),href=row.profile||'#players';return `<a class="game-line-row ${row.official?'official':''}" href="${esc(href)}"><span><strong>${esc(row.subject)}</strong><small>${esc(row.position||'Other')} · ${esc(row.marketWindow||'Full game')}</small></span><span>${esc(row.direction)} ${esc(row.line??'—')} ${esc(row.market)}</span><span><strong>${esc(row.book||'Source')}</strong><small>${row.odds==null?'price unavailable':`${row.odds>0?'+':''}${esc(row.odds)}`} · ${esc(quote.label)}</small></span><b>${row.favorite?'Favorite':row.official?'Official':row.hasResearch?'Research':'Observed'}</b></a>`}).join('');
  return `<div class="game-line-catalog">${cards}</div><p class="record-note">Official picks appear first. Observed and research rows are references unless marked as a current quote. Open a player for recent games, charts and matchup notes.</p>`;
}
function gamePage(g){
...
function parlayBoard(picks){
  ...
  const emptyTiers=tiers.filter((_,i)=>!picks.some(p=>parlayTier(p).rank===i)).map(([name])=>name.replace(/^Tier \d · /,''));
  return `<section class="parlay-tiers">${tiers.map(([name,note],i)=>{const items=picks.filter(p=>parlayTier(p).rank===i);if(!items.length)return '';return `<section class="position-group"><h3>${name}</h3><p>${note}</p><div class="kr-published-list">${items.map((p,index)=>`<details class="kr-published-item"><summary><strong>${esc(p.title)}</strong><span>${esc(p.book||'Book unavailable')} ${FootballRecords.validOdds(p.odds)?(Number(p.odds)>0?'+':'')+p.odds:'Price unavailable'}</span></summary>${pickCard(p,index)}</details>`).join('')}</div></section>`}).join('')}${emptyTiers.length?`<p class="record-note">No ticket published for: ${emptyTiers.map(esc).join(' · ')}.</p>`:''}</section>`;
}
