...
function research(kind){
  ...
  const watchCards=kind==='props'&&watchItems.size?`<section class="watch-list"><h3>Early looks · follow the research</h3><p>See the number, matchup and remaining checks before a candidate becomes an official pick.</p><div class="early-grid">${[...watchItems.values()].sort((a,b)=>Number(Boolean(b.position))-Number(Boolean(a.position))).map(earlyWatchCard).join('')}</div></section>`:'';
