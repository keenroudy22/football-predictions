...
function card(g){
  ...
  const insight=related.length?related.map(x=>`<li><strong>${esc(x.title)}</strong> · ${FootballRecords.favorite(x)?'Daily favorite · ':''}${esc(x.result||pickStatus(x))}${x.why?`<br>${esc(x.why)}`:''}${x.risk?`<br>Risk: ${esc(x.risk)}`:''}</li>`).join(''):'<li>No reviewed player prop for this game yet.</li>';
  ...
  return `<article class="game"><div class="game-top"><span>${g.timeValid?fmt(g.kickoff,{hour:'numeric',minute:'2-digit'})+' ET':'Time TBD'}</span><span class="tag ${final?'final':''}">${label}</span></div>${side('away')}${side('home')}<div class="game-bottom"><span>${final?'Provider-reported final':p?`Projected total ${p.home+p.away}`:'Forecast pending'}</span><span>${g.neutral?'Neutral site':'@ '+esc(g.home.abbreviation)}</span></div>${market}${movement}<details><summary>Game breakdown & player props</summary><h4>Score outlook</h4>${detail}<h4>Selected player props</h4><ul>${insight}</ul><p>Injury, usage and matchup notes appear with reviewed picks when verified; the baseline forecast does not include them.</p>${links([g.source])}</details></article>`;
}
function gameList(){
...
function wagerRecord(){
  ...
  const line=(name,picks)=>{const s=FootballRecords.summarize(picks);return `<tr><th scope="row">${name}</th><td>${s.wins}–${s.losses}–${s.pushes}</td><td>${s.hitRate==null?'—':s.hitRate.toFixed(1)+'%'}</td><td>${s.pending}</td><td>${s.priced?`${s.units>=0?'+':''}${s.units.toFixed(2)}u`:'—'}</td><td>${s.roi==null?'—':s.roi.toFixed(1)+'%'}</td><td>${s.graded}</td></tr>`};
  const groups=['Straights','Parlays','Longshots'];
  ...
  const pickRows=all.map(p=>{const priced=FootballRecords.validOdds(p.originalOdds),u=p.result==='win'&&priced?FootballRecords.profit(p.originalOdds):p.result==='loss'&&priced?-1:null;return `<tr><td>${esc(p.title)}${FootballRecords.favorite(p)?' <span class="tag">Favorite</span>':''}</td><td>${esc(FootballRecords.category(p))}</td><td>${esc(p.result==='unverified'?label(p):p.result||label(p))}</td><td>${priced?`${p.originalOdds>0?'+':''}${esc(p.originalOdds)} ${esc(p.originalBook||'')}`:'—'}</td><td>${u==null?'—':`${u>=0?'+':''}${u.toFixed(2)}u`}</td><td>${links([p.resultSource])}</td></tr>`}).join('');
  return `<h3>Official pick record</h3><p>Each selection risks 1 unit. Hit rate excludes pushes, voids and pending picks. Profit and ROI include only settled picks with an original recorded price. A favorite appears in the overall record once.</p>${filters}<div class="record-table"><table><thead><tr><th>Type</th><th>W–L–P</th><th>Hit</th><th>Pending</th><th>Units</th><th>ROI</th><th>Graded</th></tr></thead><tbody>${rows}</tbody></table></div><p class="record-note">${unpriced} graded result${unpriced===1?'':'s'} lack original odds, so their units and ROI are unavailable. No actual wagers are recorded.</p>${all.length?`<h4>Selections in this view</h4><div class="record-table"><table><thead><tr><th>Pick</th><th>Type</th><th>Result</th><th>Original price</th><th>Units</th><th>Source</th></tr></thead><tbody>${pickRows}</tbody></table></div>`:''}${pending.length?`<h4>Settlement queue</h4><ul class="settlement-list">${pending.map(p=>`<li><strong>${esc(p.title)}</strong> · ${esc(label(p))}${p.lastCheckedAt?` · Checked ${pretty(p.lastCheckedAt)} ET`:''}${p.nextReviewAt?` · Next review ${pretty(p.nextReviewAt)} ET`:''}<br>${esc(p.actual||'Final player statistics pending')}${links([p.resultSource])}</li>`).join('')}</ul>`:''}`;
}
