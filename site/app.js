...
async function init(){
  ...
  try{
    ...
    document.addEventListener('click',e=>{const a=e.target.closest('a[href^="#"]');if(!a)return;const route=a.getAttribute('href').slice(1);if(route.startsWith('game/')||['scores','props','parlays','record'].includes(route)){e.preventDefault();location.hash=route;navigate()}});
    ...
    const setWeek=e=>{state.week=e.target.value;render()};
