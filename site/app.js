...
let state={league:localStorage.getItem('football-league')||'NFL',view:'scores',week:weekOf(new Date()),query:'',filter:'all',propGame:'all',propPosition:'all',recordScope:'favorites',recordLeague:'NFL',recordWeek:'total',slate:null,forecasts:[],reports:[],history:null,parlaySelected:new Set(),parlayTarget:200};
...
function research(kind){
  ...
  const text=kind==='props'?'Up to five qualifying props. Fewer when the evidence says pass.':'Published tickets ordered by relative risk, then your personal builder. Lower risk still carries a chance of losing; tiers are qualitative.';
...
function parlayTier(p){
  if(p.longshot===true||/longshot/i.test(p.title)||Number(p.odds)>=1000)return {rank:2,label:'Longshot'};
  if(p.riskTier==='lower')return {rank:0,label:'Lower relative risk'};
  return {rank:1,label:'Higher / unclassified risk'};
}
function builderEligible(p){
  return pickStatus(p)==='active' && ['props','riskyProps'].includes(p.kind) &&
...
async function init(){
  ...
  try{
    ...
    document.addEventListener('click',async e=>{if(e.target.id==='copy-ticket'){try{await navigator.clipboard.writeText($('#gambly-ticket').value);$('#copy-status').textContent='Copied. Open Gambly and paste your ticket.'}catch(error){$('#gambly-ticket').select();$('#copy-status').textContent='Select and copy the ticket text above.'}return}const parlay=e.target.closest('[data-parlay-pick]');if(parlay){const id=parlay.dataset.parlayPick;state.parlaySelected.has(id)?state.parlaySelected.delete(id):state.parlaySelected.add(id);render();return}const load=e.target.closest('[data-parlay-ids]');if(load){state.parlaySelected=new Set(load.dataset.parlayIds.split(',').filter(Boolean));render();return}if(e.target.id==='clear-prop-filters'){state.propGame='all';state.propPosition='all';render();return}const a=e.target.closest('a[href^="#"]');if(!a)return;const route=a.getAttribute('href').slice(1);if(route.startsWith('game/')||['scores','props','parlays','record'].includes(route)){e.preventDefault();location.hash=route;navigate()}});
    ...
    $('#content').addEventListener('change',e=>{if(e.target.id==='parlay-target'){state.parlayTarget=Number(e.target.value)||200;render();return}if(e.target.id==='game-filter'){state.filter=e.target.value;$('#game-list').innerHTML=gameList()}else if(e.target.id==='prop-game'){state.propGame=e.target.value;render()}else if(e.target.id==='prop-position'){state.propPosition=e.target.value;render()}else if(['record-scope','record-league','record-week'].includes(e.target.id)){state[{"record-scope":"recordScope","record-league":"recordLeague","record-week":"recordWeek"}[e.target.id]]=e.target.value;render()}else if(e.target.closest('.calculator'))updateCalculator(e.target.closest('.calculator'))});
